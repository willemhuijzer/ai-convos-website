...
- This repository's `github-personal` SSH host authenticates as the GitHub account `QST23`. Git pushes can therefore succeed even when GitHub CLI defaults to `QuinnO8`, which is not an upstream collaborator and cannot create a same-repository pull request.
- Before opening a pull request, check the active CLI identity with `gh api user --jq .login`.
- If it is not `QST23`, run `gh auth switch --hostname github.com --user QST23`. This selects an existing keyring login; do not add tokens or credentials to the repository.
- Push the branch to `origin`, then create the pull request with an explicit repository: `gh pr create --repo willemhuijzer/ai-convos-website --base main --head <branch>`.
- If creation reports that the caller must be a collaborator, re-check the active CLI identity before attempting a fork or concluding that pull-request creation is unavailable.
